"""
Here we want to approximate the price of an American put option with maturity T and strike K, written on an underlying
following the Black-Scholes model (i.e., having log-normal dynamics) with log-volatility sigma and risk-freee rate r. It
can be seen that such a model can be approximated by a binomial model with N times, for N large enough,
u = exp(sigma*sqrt(T/N)), d=1/u, rho = exp(rT/N)-1.
We then valuate the option under such a binomial model and hope that the price converges for large N.
