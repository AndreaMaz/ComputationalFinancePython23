...
class AmericanOptionWithControlVariates:
    """
    This class is devoted to an application of control variates to the approximation of the price of an american put
    option under the Black-Scholes model, simulating a binomial model.
    
    In particular, we rely on: 
        - the analytic price P_E of an european put under the Black-Scholes model
        - the approximated price P_E^N of an european put under the Black-Scholes model, computed using a binomial model
         with N times
        - the approximated price P_A^N of an american put under the Black-Scholes model, computed using a binomial model
        with N times
          
    Using the heuristic that P_A - P_A^N is approximately equal to P_E - P_E^N, we then approximate P_A as
    P_A^N + P_E - P_E^N
    ...
    r : float
        the risk free rate of the model, i.e., such that the risk free asset B has dynamics B(t)=exp(r t)
    sigma: float
    ...
    getEuropeanCallAndPutPriceWithBinomialModel(numberOfTimes):
        It returns the approximated value of an European put option written on a Black-Scholes model, by using a
        binomial model with numberOfTimes times

    getAmericanCallAndPutPriceWithBinomialModel(numberOfTimes):
        It returns the approximated value of an American put option written on a Black-Scholes model, by using a
        binomial model with numberOfTimes times
        
    getAmericanCallAndPutPriceWithControlVariates(self, numberOfTimes):
        It returns the approximated value of an american put option written on a Black-Scholes model, by using
        control variates
    """
    ...
    def getAmericanPutPriceWithBinomialModel(self, numberOfTimes):
        ...
        binomialModel = BinomialModelSmart(initialValue, decreaseIfDown, increaseIfUp, numberOfTimes, interestRate)
    
        payoffEvaluator = AmericanOption(binomialModel)
        
        payoffPut = lambda x : max(strike - x, 0)
        
        approximatedPriceAmericanPut = payoffEvaluator.getValueOption(payoffPut, numberOfTimes - 1)
        
        return approximatedPriceAmericanPut


    def getEuropeanPutPriceWithBinomialModel(self, numberOfTimes):
        """
        It returns the approximated value of an european put option written on a Black-Scholes model, by using a binomial
        model with numberOfTimes times
    ...
    def getAmericanPutPriceWithControlVariates(self, numberOfTimes):
        """
        It returns the approximated value of an american put option written on a Black-Scholes model, by using a binomial
        model with control variates
