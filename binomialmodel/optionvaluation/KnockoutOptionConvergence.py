"""
Here we want to approximate the price of a Knock-out, down-and-out call option with maturity T and strike K, written on
an underlying following the Black-Scholes model (i.e., having log-normal dynamics) with log-volatility sigma and risk-free
rate r. It can be seen that such a model can be approximated by a binomial model with N times, for N large enough,
u = exp(sigma*sqrt(T/N)), d=1/u, rho = exp(rT/N)-1.
We then evaluate the option under such a binomial model and hope that the price converges for large N.
...
vectorNumberOfTimes = range(2, maximumNumberOfTimes + 1)
...
for numberOfTimes in range(2, maximumNumberOfTimes + 1):
    ...
    prices.append(myPayoffEvaluator.getInitialDiscountedValueOption(payoff, numberOfTimes - 1, lowerBarrier))

blackScholesPrice = blackScholesDownAndOut(initialValue, r, sigma, maturity, strike, lowerBarrier)

plt.plot(vectorNumberOfTimes, prices)
blackScholesVector = [blackScholesPrice] * (maximumNumberOfTimes - 1)
plt.plot(vectorNumberOfTimes, blackScholesVector)
