...
class BinomialModelSmartWithLists(BinomialModel):
    """
    In this class we implement the simulation of a binomial model by using an approach that is computationally more
    effecient than Monte-Carlo, since it needs much less memory, and most of all is not prone to the simulation problems
    that Monte Carlo exhibits.

    In particular, here we don't rely on pseudo random numbers generations, but at a given time we just consider ALL
    the possible realizations of the process attributing a (analytic!) probability to every realization. We then compute
    ...
    getRealizations()
        It returns a list representing all the possible realizations of the process up to time self.numberOfTimes - 1.
   ...
   findThreshold(timeIndex):
        It returns the smallest integer k such that (u)^kd^(timeIndex-k) > (1+rho)^N, i.e., such that the realization of
        the process given by k ups and timeIndex - k downs, discounted at time 0, is bigger than the initial value.
    """
    ...
    def generateRealizations(self):
        ...
        for k in range(1, self.numberOfTimes):
            ...
            # last one times d. Here we cannot multiply directly by the vector, because it is a list
            realizationsAtCurrentTime = [self.decreaseIfDown * x for x in realizations[k - 1]]
            # the kth element of the list is a list representing the realizations at time k: look at how we concatenate
            # a list which is actually a singleton with another list
            realizations.append([highestRealization] + realizationsAtCurrentTime)
        return realizations
    ...
    def getRealizations(self):
        ...
        The realizations are stored in a list which represents a triangular matrix
        ...
        realizations : list
            ...
            This is a kind of particular matrix: the first row has length 1, the second 2, and so on.
    ...
    def getRealizationsAtGivenTime(self, timeIndex):
        ...
        # We return the element timeIndex of our list: this is the timeIndex-th row of the triangular matrix
    ...
    def getPercentageOfGainAtGivenTime(self, timeIndex):
        ...
        ----------
        timeIndex : int
            The time j at which we want the percentage probability that (1+rho)^(-j)S(j)>S(0) with rho = self.interestRate

        Returns
        -------
        float
            the percentage probability that (1+rho)^(-timeIndex)S(timeIndex)>S(0) with rho = self.interestRate
    ...
    def getDiscountedAverageAtGivenTime(self, timeIndex):
        ...
        if timeIndex == 0:
            ...
            # we discount the weighted sum of the realizations  use np.dot(a,b) to get the scalar product of two vectors
            # arrays a, b
