...
class BinomialModel(metaclass=abc.ABCMeta):
    """
    This is an abstract class whose main goal is to get the main features of a binomial model, like the evolution of its
    average and of its realizations. It can be extended by classes providing the way to construct it
    ...
    ...
    decreaseIfDown : float
        the number d such that S(j+1)=dS(j) with probability 1 - q. It must be strictly smaller than 1 + rho.
    increaseIfUp : float
        the number u such that S(j+1)=uS(j) with probability q. It must be strictly biggler than 1 + rho.
    ...
    getPercentageOfGainAtGivenTime(timeIndex)
        It returns the percentage probability that (1+rho)^(-j)S(j)>S(0) with j = timeIndex
    ...
    def __init__(self, initialValue, decreaseIfDown, increaseIfUp,
        ...
        decreaseIfDown : float
            the number d such that S(j+1)=dS(j) with probability 1 - q. It must be strictly smaller than 1 + rho.
        increaseIfUp : float
            the number u such that S(j+1)=uS(j) with probability q. It must be strictly bigger than 1 + rho.
        numberOfTimes : int
            the number of times at which the process is simulated, initial time
            included
        interest rate : float
    ...
    # note the syntax: this is an abstract method, whose implementation will be given in the derived classes.
    # In our case, we will see the implementation with a pure Monte Carlo method and a smarter one.
    @abc.abstractmethod
    ...
    def getDiscountedAverageAtGivenTime(self, timeIndex):
        """It computes the average of the realizations of the process at timeIndex, discounted at time 0
        """
    ...
    def getEvolutionDiscountedAverage(self):
        ...
        -------
        list
            A vector representing the evolution of the average of the process discounted at time 0, for j going from 1
            to self.numberOfTimes - 1
        """

        return [self.getDiscountedAverageAtGivenTime(timeIndex) for timeIndex in range(self.numberOfTimes)]

    def printEvolutionDiscountedAverage(self):
        """
        It prints the evolution of the average of the process discounted at time 0, for j going from 1 to
        self.numberOfTimes - 1
        """
        ...
        print()
        # note the syntax to tell the program we want to print three decimal digits, separating the strings with a comma
        print('\n'.join('{:.3}'.format(discountedAverage) for discountedAverage in evolutionDiscountedAverage))
        print()

    def plotEvolutionDiscountedAverage(self):
        """
        It plots the evolution of the average of the process discounted at time 0, for j going from 1 to
        self.numberOfTimes - 1
        """
        evolutionDiscountedAverage = self.getEvolutionDiscountedAverage();
    ...
    def getPercentageOfGainAtGivenTime(self, timeIndex):
        ...
        -------
        float
            The percentage probability that (1+rho)^(-timeIndex)S(timeIndex)>S(0)
        """
    ...
    def getEvolutionPercentageOfGain(self):
        ...
        -------
        list
            A list representing the evolution of percentage that (1+rho)^(-j)S(j)>S(0), for j going from 1 to
            self.numberOfTimes - 1 .
        """

        return [self.getPercentageOfGainAtGivenTime(timeIndex) for timeIndex in range(self.numberOfTimes)]

    def printEvolutionPercentagesOfGain(self):
        """
        It prints the evolution of the percentage that (1+rho)^(-j)S(j)>S(0), for j going from 1 to
        self.numberOfTimes - 1
        """
    ...
    def plotEvolutionPercentagesOfGain(self):
        """
        "It plots the evolution of the percentage that (1+rho)^(-j)S(j)>S(0), for j going from 1 to
        self.numberOfTimes - 1
        """
        percentagesPath = self.getEvolutionPercentageOfGain();
