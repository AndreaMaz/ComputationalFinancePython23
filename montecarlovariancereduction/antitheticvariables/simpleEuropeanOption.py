...
class SimpleEuropeanOption:
    ...
    getPrice(payoffFunction):
        It returns the price of the option defined by payoffFunction, valuated in the realizations of the process stored
         in self.processRealizations.
    printPrice(payoffFunction):
        It prints the price of the option defined by payoffFunction, valuatedin the realizations of the process stored
        in self.processRealizations.
    """
    ...
    def getPrice(self, payoffFunction, maturity):
        """
        It returns the discounted price of the option defined by payoffFunction and payed at maturity. The underlying at
        maturity is defined by the realizations of the process stored in self.processRealizations.
        ...
        #look at the use of numpy.mean: we get the average of the elements of a list, or array, of numbers
        return math.exp(-self.r * maturity) * mean(payoffRealizations)
    
    
    def printPrice(self, payoffFunction, maturity):
        """
        It prints the discounted price of the option defined by payoffFunction and payed at maturity. The underlying at
        maturity is defined by the realizations of the process stored in self.processRealizations.
