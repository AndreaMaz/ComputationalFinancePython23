...
class FasterControlVariatesCliquetBSWithArrays:
    ...
    The payoff of the Cliquet option with local floor localFloor, local cap localCap, global floor globalFloor,
    global cap globalCap and partition of the interval
    ...
    maturity : float
        the maturity of the option. It is also the final time of the global time interval, i.e., the last time of the partition
    ...
    def getDiscountedPriceOfTheOption(returnsForAllSimulations, interestRate):
        It returns the discounted price of the Cliquet option, as the discounted average of the payoffs
    """

    def __init__(self, numberOfSimulations, maturity, numberOfIntervals, localFloor, localCap, globalFloor, globalCap,
                 sigma, r):
        """
        Parameters
        ----------
    ...
    def getAnalyticPriceOfNonTruncatedSum(self):
        ...
        secondCallPrice = blackScholesPriceCall(initialValue, self.r, self.sigma, maturityOfTheCallOptions,
                                                secondStrike) \
                          / discountFactorBlackScholes
    ...
    def getPriceViaControlVariates(self):
        ...
        In particular, the Control variates method is performed using the analytic price of the option with payoff
        ...
        generator = GenerateBSReturnsWithArrays(numberOfSimulations, numberOfIntervals, T, sigma, r)

        returnsRealizations = generator.generateReturns()

        cliquetOption = CliquetOptionForCVWithArrays(numberOfSimulations, T, lF, lC)

        # First we get the Monte-Carlo prices of the option, both for the truncated and not truncated sum. First of all,
        # we can use a single object to do the valuations. Moreover, thanks to the fact that we generate the truncated
        # returns only once, we basically halve the computation time
        discountedPriceOfTheOptionMC = cliquetOption.getDiscountedPriceOfTheOption(returnsRealizations, r, gF, gC)

        discountedPriceNonTruncatedSumMC = cliquetOption.getDiscountedPriceOfTheOption(returnsRealizations, r)
        ...
        return discountedPriceOfTheOptionMC - optimalChoice * (
                    discountedPriceNonTruncatedSumMC - analyticPriceOfNonTruncatedSum)
