...
from optimalEmissions import minimizeExpectedLoss
...
probabilityFunctions = lambda a : np.array([0.1*(1-math.atan(0.5*a)), 0.1*(1-math.atan(0.5*a)), 0.5*(1-math.atan(0.5*a)),
                                   0.9*(1-math.atan(0.5*a)),  0.9*(1-math.atan(0.5*a))])
...
ambiguityAversionFunction = lambda x : x**1.9 #note that it is convex!

#weightsExperts = [1/5,1/5,1/5,1/5,1/5]
#weightsExperts = [1/4,1/8,1/4,1/8,1/4]
...
optimalAbatement = minimizeExpectedLoss(lossFunction, consumptionFunction, probabilityFunctions,
                                           ambiguityAversionFunction, weightsExperts, probabilitiesGivenTippingPoint,
